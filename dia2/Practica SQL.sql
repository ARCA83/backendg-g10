...
INSERT INTO almacen_producto (id, almacen_id, productos_id) VALUES (DEFAULT, 1, 1),
                                                                  (DEFAULT, 1, 2),
                                                                  (DEFAULT, 2, 2),
                                                                  (DEFAULT, 2, 3),
                                                                  (DEFAULT, 3, 4),
...
-- 2 Mostar todos los productos cuya fecha de vencimiento sea menor al 1 agosto del 2022
SELECT * FROM productos WHERE fecha_vencimiento < '2022-08-1';
...
-- 5 Devolver el nombre de la categoria, nombre del producto y el precio del producto si es mayor a 10 soles
...
FROM almacenes AS a INNER JOIN almacen_producto AS ap ON a.id = ap.almacen_id
					INNER JOIN productos AS p ON ap.productos_id = p.id;
                    
-- 7. Mostrar los siguiente
-- Tabla categoria: *nombre
-- Tabla Productos: *nombre/*precio
-- Tabla Almacenes: *nombre/*direccion
SELECT c.nombre,p.nombre,p.precio,a.nombre,a.direccion
FROM categorias AS c
INNER JOIN productos AS p ON c.id = p.categoria_id
INNER JOIN almacen_producto AS ap ON  p.id = ap.productos_id
